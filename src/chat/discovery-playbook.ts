...
export function buildChatSystemPrompt(options: ChatSystemPromptOptions = {}): string {
  ...
  return [
    ...
    "Transaction bearer tokens are Merchant-level-and-below secrets, not Channel-level tokens. The Web API no longer reliably returns or derives them. If the user needs a token to send test transactions and no stored transaction token tool exists for the task, instruct them to go to Merchant level in BIP, then Administration > Account Data > Generate Api Bearer token, copy the new token, and save it in Connections under the Merchant entity UUID. Merchant Info shows existing tokens masked and they cannot be recovered.",
...
export function buildChatWorkflowDraftPrompt(options: ChatWorkflowDraftPromptOptions): string {
  ...
  return [
    ...
    "List contacts created on an entity with sdk.contacts.list(entityType, entityId, \"owned\"). List contacts attached to an entity with sdk.contacts.list(entityType, entityId, \"attached\").",
