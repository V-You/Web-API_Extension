...
export function buildChatWorkflowDraftPrompt(options: ChatWorkflowDraftPromptOptions): string {
  ...
  return [
    ...
    "Universal list contract: every sdk.*.list*, sdk.*.search, and sdk.entities.listChildren method returns a plain JavaScript array of row objects. Call .map / .filter / .slice / .find directly. Do not read .data, .items, .ownedContacts, .merchantAccounts, or any other wrapper key off the return value - the SDK normalizes the envelope for you.",
